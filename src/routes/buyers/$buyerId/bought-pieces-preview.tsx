...
export const Route = createFileRoute('/buyers/$buyerId/bought-pieces-preview')({
  component: BoughtPiecesPreviewList,
})

function BoughtPiecesPreviewList() {
  ...
  const columns = useMemo<ColumnDef<WoodPiece>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (info) => (
          <div className="text-center">{info.getValue() ? t('yes') : t('no')}</div>
        ),
      },
    ],
    [],
  ...
  const exportToFile = async () => {
    const path = await save({
      ...
      defaultPath: `${t('boughtPiecesPreviewPDFName')}-${slugifyFilenamePart(buyer.buyer_name)}`,
    })
    let toastId: string
    if (path) {
      ...
      try {
        await saveToPDF(
          ...
          PdfTypeEnum.boughtPiecesPreview,
          i18n.language,
        )
      } catch (e) {
